...
In addition [Fixedwing Airmode](https://github.com/iNavFlight/inav/wiki/Modes#air-mode) can be enabled with `ACRO` mode. Do this before tuning, because the airplane will react differently to the Integral term.
...
* To calculate an axis _(approximate)_ rate from a DVR recording, you'll need to count the number of frames it took for your aircraft to do a complete maneuver (roll/flip/yaw turn), determined by the average FPS of the recording. Then use this formula **:** `360 / (number_of_frames / FPS)`.   
