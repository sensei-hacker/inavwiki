...
## JUMP, HOLD and LAND Waypoint types support ([@giacomo892], [@Jacky2k], [@stronnag])

New Waypoints types supported by INAV. Now INAV can perform much more complex missions!
* **JUMP**: Allows set the mission to go to a specific waypoint number, allowing mission loops.
* **HOLD**: Allows set the amount of time the aircraft will hold the waypoint position before proceeding to the next waypoint.
* **LAND**: Tells INAV that aircraft must land on that waypoint. It allows INAV to perform one-way missions.
