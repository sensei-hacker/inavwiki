...
# Parse release targets for sensors etc.
# (c) Jonathan Hudson
...
Find.find('.').select { |f| f =~ /target\.h$/ }.each do |fn|
  ...
  cfn = "#{target}/CMakeLists.txt"
  next unless File.exist?(cfn)
  next unless check_rel(cfn)
...
The following table was machine generated by [find_sensors.rb](assets/find_sensors.rb) script on %s against INAV %s, E&OE
