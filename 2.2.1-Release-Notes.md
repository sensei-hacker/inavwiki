...
|  `imu_acc_ignore_rate ` | Rotation rate above which accelerometer is no more used in IMU on airplanes |
|  `imu_acc_ignore_slope ` | When imu_acc_ignore_slope is not zero it will create a band of width 2 * imu_acc_ignore_slope centered around imu_acc_ignore_rate. Below that band accelerometer weight scale will be 1.0, above - 0.0. In that band it will linearly change. |
### Removed
