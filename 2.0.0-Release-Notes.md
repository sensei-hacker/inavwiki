...
**STM32F1 boards support has been dropped. NAZE/CC3D targets are no more supported.** Please upgrade to F4/F7 board. F3 boards are not suggested since their flash space is almost full and they are likely to not receive all features on the next release.
...
* __Experimental__ Terrain Following mode (aka `SURFACE` mode). If `AltHold` mode is enabled together with `SURFACE` mode, UAV will try keep ground altitude reported by rangefinder instead for baro/GPS absolute altitude.  [#3202]
