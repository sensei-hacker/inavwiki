...
| `manual_rc_expo` | Expo settings used for the PITCH/ROLL axes by the `MANUAL` flight mode [0-100]. Default is 70 |
| `manual_rc_yaw_expo` | Expo settings used for the YAW axis by the `MANUAL` flight mode. Default is 20 |
...
| `manual_yaw_rate` | Servo travel multiplier for the ROLL axis in `MANUAL` flight mode [0-100]%. Default is 100% |
...
| `smartport_fuel_unit` | Unit of the value sent with the `FUEL` ID through the S.Port telemetry [PERCENT/MAH/MWH]. Default is PERCENT |
