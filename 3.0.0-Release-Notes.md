...
4. Go the CLI again and paste the contents on the file you previously created and write _save_ , press ENTER.
...
# pitch trim should be done using `fw_level_pitch_trim` setting. +5 `fw_level_pitch_trim` means airplane nose should be raised 5 deg from level
# Flight mode `AUTO LEVEL` will automatically trim the pitch angle when ANGLE or HORIZON flight modes are used
