...
* **Not available on boards with 256KB of RAM or less** (F405, F411, F722, and similar) — there isn't enough spare RAM for this feature, and calibration works exactly as it always has. Use the [Alignment Tool Tab](https://inavflight.github.io/docs/04-inav-configurator/alignment-tool-tab)'s wizard instead, which determines board *and* compass orientation together in one guided flow on these boards.
* If the spin doesn't produce a confident result (too few samples, or an ambiguous spin), INAV leaves your existing alignment setting untouched rather than guessing — use the [Alignment Tool Tab](https://inavflight.github.io/docs/04-inav-configurator/alignment-tool-tab)'s manual compass wizard fallback, or fall back to the manual methods below.

See [Alignment Tool Tab](https://inavflight.github.io/docs/04-inav-configurator/alignment-tool-tab) on the INAV docs site for the full wizard walkthrough and which order to do board vs. compass alignment in, depending on your board.
