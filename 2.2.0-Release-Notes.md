...
1. Save to a file the current _diff_ from the CLI.
2. Upgrade to INAV 2.2 using the Full Erase option in the configurator.
3. Go the CLI again and paste the contents on the file you previously create and write _save_ , press ENTER and done!
4. If you armed with via sticks, add an arming switch in the Modes tab in the configurator.
5. If you are upgrading a multi rotor, POS XY PID I and D have now specific settings, respectively `nav_mc_pos_deceleration_time` and `nav_mc_pos_expo` . So if you don't use defaults, when restoring, move yours to the new settings.
6. You should be ready, explore new 2.2 features and enjoy!
...
* Added option to continue mission out of radio range ([@shellixyz] ,[#4713])
